...
Introduction
------------
Structured Query Language source Management Services
...
all sql scripts that have the configured file extension. Finally batch and execute 
all scripts in the correct order. 

Future versions will then attempt to interpret all sql scripts for syntax errors
before execution.

---

**Boilerplate Service**: Use freemarker templates and properly annotated entity 
...
stored procedures to perform all database operations. 
+ Delete Procedure: Uses the record id to perform a soft delete on that record.
+ Insert Procedure: Inserts a record.
+ Select Procedure: By parsing each of entity fields annotations we capture any 
                    field that is unique and can thus be used to query the data 
                    record. Those fields are then each used to generate a stored 
                    procedure which queries that datasource by its value. The 
                    naming convention used when generating queries is select${Table}By${Field}.
+ Update Procedure: Performs table update.
+ Upsert Procedure: Using any or all of a tables unique columns verify their 
                    presence to determine either an update or insert event.
...
This set of triggers are created for each table:
+ AfterInsertTrigger: Insert records for each column into the autit table.
+ BeforeDeleteTrigger: Intercepts delete call and instead performs a soft delete by
                       populating the delete date field which is checked by all 
                       select routines before returning records.
+ BeforeUpdateTrigger: This trigger checks each parameters value against the current
                       columns value and only inserts records into the audit table
                       for columns that have updated values.

The tables generated by the BoilerplateService also part of the audit framework.
+ Audit.ftl: Records an individual columns crud event, value, invoker and creation
             date.
...
**Rollback Service**: Attempt to undo the last build by parsing the Build Report
